...
class PipelineRunner:
    ...
    Auto-scheduling
        ...
        algorithm).  Circular dependencies raise :class:`ValueError` at
        construction time.
    Caching
    ...
    def run(
        ...
        for step in self._ordered_steps:
            ...
            cached = self._state_manager.load_intermediate_state(
                request_id, output_key, step.schema_type
            )
            if cached is not None:
                logger.info("[Pipeline] Loaded '%s' from cache.", output_key)
            ...
            if step.use_json_retry:
                ...
                result = self._json_retry.parse_with_retry(
                    parser_func=lambda s=step, i=inputs: s.run(**i),
                    schema_name=output_key,
                    generator=generator,
                )
                if result is None:
                    raise RuntimeError(
                        f"Pipeline step '{output_key}' failed after all retries."
                    )
            else:
                result = step.run(**inputs)

            self._state_manager.save_intermediate_state(
                request_id, output_key, result
            )
            state[output_key] = result

        return state

    def clear(self, request_id: str, *output_keys: str) -> None:
        ...
        *output_keys : str
        ...
        for key in output_keys:
            self._state_manager.clear_intermediate_state(request_id, key)
    ...
    def _topological_sort(
        ...
        """
        output_to_step: dict[str, PipelineStep] = {
            step.output_key: step for step in steps
        }

        in_degree: dict[str, int] = {step.output_key: 0 for step in steps}
