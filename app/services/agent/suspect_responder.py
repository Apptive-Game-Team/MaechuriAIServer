...
from app.models.schemas.suspect_responder import SuspectResponderOutput
...
class SuspectResponder:
    ...
    async def arespond(
        ...
        Returns:
            SuspectResponderOutput: pressure_delta + reasoning + response
        """
        ...
        raw = await self.llm.acomplete(
            ...
            user=user_message,
            response_schema=SuspectResponderOutput.model_json_schema()
        )

        json_text = extract_json(raw)
        data = safe_json_load(json_text)

        return SuspectResponderOutput.model_validate(data)
